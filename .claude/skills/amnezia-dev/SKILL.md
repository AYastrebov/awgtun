...
name: amnezia-dev
description: >
  AmneziaWG 2.0 development skill for the boringtun fork. Covers protocol
  ...
  "amneziawg-go comparison", "interop test", or any mention of H1-H4, S1-S4,
  Jc, Jmin, Jmax, I1-I5 parameters.
---

# AmneziaWG 2.0 Development Guide

This skill provides context for developing, debugging, auditing, and maintaining
the AmneziaWG 2.0 protocol implementation in this boringtun fork.
...
| Padding | S1-S4 (u8, bytes) | Random prefix bytes. Added AFTER MAC — not authenticated. S4 skipped for keepalives. |
...
Outbound:
  ...
  4. Send: [padding][authenticated WG message]

Inbound:
  1. determine_padding() — try each type's padding+size+header combo
  2. Strip padding → get clean WG message
  3. Parse header, verify MAC, process message
```
...
noise/timers.rs     — network_outgoing.clear() on reset
ffi/mod.rs          — C FFI: amnezia_config, new_tunnel_amnezia, poll_outgoing
```
...
2. **Check these critical behaviors match:**
   ...
   - S4 skipped for keepalive (`len(elem.packet) != MessageKeepaliveSize`)
...
3. Check keepalive handling — S4 should be 0 for empty payloads
...
- `encapsulate()` needs `src.len() + 32 + S4` bytes
...
5. Update FFI `amnezia_config` struct and `new_tunnel_amnezia`
6. Update `AMNEZIA.md` and `CLAUDE.md`
...
`cookie.go`, `uapi.go`, `magic-header.go`, `obf.go`
